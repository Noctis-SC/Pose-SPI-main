...
from models.model_grayscale_keypointrcnn import KeypointDetection, KeypointRCNNLoss
...
parser.add_argument("--output_dir", default="", help="Path to save weights ")
...
# FYI for validation both mse or keypointrcnn loss can be used. Depending on needs
criterion = KeypointRCNNLoss(num_keypoints=args.num_keypoints)
...
for epoch in range(args.epochs):
    ...
    for idx, (images, keypoints, image_id) in enumerate(train_data_loader):
        ...
        if idx % 50 == 0:
            ...
            summary.add_scalar("loss/train", loss.item(), global_step=epoch)
    ...
    with torch.no_grad():
        for val_idx, (images, keypoints, _) in enumerate(val_data_loader):
            ...
            outputs = model(images)
            loss = val_criterion(outputs, keypoints)
